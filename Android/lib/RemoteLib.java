...
public class RemoteLib {
     ...
     * @param finishHandler
     */
    public void uploadBookImage(int infoSeq, File file, Handler finishHandler) {
        RemoteService remoteService = ServiceGenerator.createService(RemoteService.class);
        ...
        Call<ResponseBody> call =
                remoteService.uploadBookImage(infoSeqBody, body);
        call.enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call,
                                   Response<ResponseBody> response) {
                MyLog.d(TAG, "uploadBookImage success");
            }

            @Override
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                MyLog.e(TAG, "uploadBookImage fail");
            }
        });
    }
}
