...
/**
 * 도서정보 메인 액티비티이며, 왼쪽에 내비게이션 뷰를 가지며
...
public class MainActivity extends AppCompatActivity
    ...
    /**
     * 액티비티와 내비게이션 뷰를 설정하고 BookListFragment를 화면에 보여준다.
     * @param savedInstanceState 액티비티가 새로 생성 되었을 경우에 이전 상태 값을 가지는 객체
     */
    @Override
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_list) {
            GoLib.getInstance().goFragment(getSupportFragmentManager(),
                    R.id.content_main, BookListFragment.newInstance());

        } else if (id == R.id.nav_keep) {
            GoLib.getInstance().goFragment(getSupportFragmentManager(),
                    R.id.content_main, BookKeepFragment.newInstance());

        } else if (id == R.id.nav_register) {
            GoLib.getInstance().goBookRegisterActivity(this);
